...
读取涉及仓库对应的 references/dig-repo-log/<repo>/supplementary_analysis.md 的 Part 1(热点文件风险分析)，确认:
- 目标文件是否在高风险文件列表中
- 目标文件所属模块的缺陷密度
...
根据涉及的仓库，Read对应目录下的全部分析文件:
  ...
  - supplementary_analysis.md — 热点文件风险、Revert教训、跨类别系统性风险
  - defect_analysis.md — 逐条缺陷的根因、修复模式、涉及文件

仓库路由:
  - 涉及通信算法 → 读hccl-hcomm
  - 涉及算子开发 → 读ops-transformer、ops-nn
...
- vibe-dsgn: 可读取其产出的设计文档作为阶段0.1的输入
